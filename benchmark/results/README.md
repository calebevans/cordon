...
**Tests Conducted:** 10 configurations, 110 runs total  
...
2. **High variance at small sample sizes on HDFS logs**: Results on 50k-250k samples show CV > 15%, indicating instability and sensitivity to which specific portion of this repetitive, structured log file is sampled. Less repetitive log types may perform better at smaller sample sizes.

3. **Parameter tuning provides minimal benefit**: Adjusting window size and k-neighbors yields marginal improvements (~2%) that do not overcome fundamental small-sample limitations.

4. **Threshold changes affect traditional metrics only**: Varying anomaly percentile (0.02 → 0.10) impacts line-level precision/recall but leaves template-level metrics essentially unchanged.
...
│   ├── 50k_w5_k10_p02/            (baseline)
│   ├── 50k_w2_k5_p02/             (tuned parameters)
│   ├── 50k_w2_k5_p05/             (threshold: 5%)
│   └── 50k_w2_k5_p10/             (threshold: 10%)
├── 100k/                           (100,000 line tests)
│   ├── 100k_w5_k10_p02/           (baseline)
│   └── 100k_w3_k7_p05/            (tuned + threshold)
├── 250k/                           (250,000 line tests)
│   ├── 250k_w5_k10_p02/           (baseline)
│   └── 250k_w4_k8_p04/            (tuned + threshold)
├── 500k/                           (500,000 line tests)
│   └── 500k_w5_k10_p02/           (baseline)
├── 1M/                             (1,000,000 line tests)
│   └── 1M_w5_k10_p02/             (baseline)
└── 5M/                             (5,000,000 line tests)
    └── 5M_w5_k10_p02/             (baseline)
```
...
python benchmark/evaluate.py hdfs_v1 \
  ...
  --runs 10 --seed 42 \
  --generate-plots \
  --output-dir benchmark/results/100k/100k_w5_k10_p02

# Tuned test example (50k)
python benchmark/evaluate.py hdfs_v1 \
  --sample-size 50000 \
  --window-size 2 --stride 2 \
  --k-neighbors 5 --anomaly-percentile 0.02 \
  --model BAAI/bge-large-en-v1.5 \
  --device cuda --batch-size 512 \
  --runs 10 --seed 42 \
  --generate-plots \
  --output-dir benchmark/results/50k/50k_w2_k5_p02
```
...
### A.1 Baseline Configuration Results (w5_k10_p02)
...
### A.2 Parameter Tuning Results (50k)
...
### A.3 Test Configuration Summary

**Total Tests Conducted:** 10 configurations  
**Total Runs:** 110 individual runs  
**Total Compute Time:** ~48 hours (GPU: NVIDIA CUDA)  
**Total Lines Analyzed:** ~45 million (across all runs)  
**Seeds Used:** 42 (for reproducibility)  
**Model Used:** BAAI/bge-large-en-v1.5 (1024-dim embeddings, 512 token context)